...
struct OrbitSwitcherView: View {
    ...
    var body: some View {
        ZStack {
            ...
            if manager.apps.isEmpty {
                ...
                ForEach(Array(manager.apps.enumerated()), id: \.element.id) { index, appInfo in
                    ...
                    // ZStack naturally treats the center of the screen as X:0, Y:0.
                    // The top of our arc curve is -pi/2 (where sine is -1).
                    // Example Math: sin(-pi/2) * 350 + 350 = 0 (perfectly centered on your screen!).
        ...
        // CRITICAL FIX: Forces the transparent view to actually expand to fill the entire screen space
    ...
    private func calculateAngle(for index: Int, selectedIndex: Int, total: Int) -> CGFloat {
        ...
        // Wrap around (if shortest path is backward, go backward!)
        if offset > half {
            offset -= Double(total)
        } else if offset < -half {
            offset += Double(total)
